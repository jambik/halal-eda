...
@section('title')Первое@endsection

@section('content')
	<h1 class="text-center">Первое</h1>
	<p>
		<a href="{{ route('admin.meal1.create') }}" class="btn btn-success"><i class="fa fa-plus"></i> Добавить</a>
	</p>
		...
		<tbody>
			@foreach($items as $item)
				<tr>
					...
					<td>{{ $item->weight }}</td>
					<td>{{ $item->price }}</td>
					<td><a href="{{ route('admin.meal1.edit', $item->id) }}" class="btn btn-primary btn-sm"><i class="fa fa-edit"></i></a></td>
					<td>
						{!! Form::open(['url' => route('admin.meal1.destroy', $item->id), 'method' => 'DELETE']) !!}
