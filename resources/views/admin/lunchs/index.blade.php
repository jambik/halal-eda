...
@section('title')Обеды@endsection

@section('content')
	<h1 class="text-center">Обеды</h1>
	<p>
		<a href="{{ route('admin.lunchs.create') }}" class="btn btn-success"><i class="fa fa-plus"></i> Добавить</a>
	</p>
		...
		<tbody>
			@foreach($items as $item)
				<tr>
					...
					<td>{{ $item->price }}</td>
					<td><a href="{{ route('admin.lunchs.edit', $item->id) }}" class="btn btn-primary btn-sm"><i class="fa fa-edit"></i></a></td>
					<td>
						{!! Form::open(['url' => route('admin.lunchs.destroy', $item->id), 'method' => 'DELETE']) !!}
