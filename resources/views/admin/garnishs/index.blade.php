...
@section('title')Гарниры@endsection

@section('content')
	<h1 class="text-center">Гарниры</h1>
	<p>
		<a href="{{ route('admin.garnishs.create') }}" class="btn btn-success"><i class="fa fa-plus"></i> Добавить</a>
	</p>
		...
		<tbody>
			@foreach($items as $item)
				<tr>
					...
					<td>{{ $item->weight }}</td>
					<td>{{ $item->price }}</td>
					<td><a href="{{ route('admin.garnishs.edit', $item->id) }}" class="btn btn-primary btn-sm"><i class="fa fa-edit"></i></a></td>
					<td>
						{!! Form::open(['url' => route('admin.garnishs.destroy', $item->id), 'method' => 'DELETE']) !!}
