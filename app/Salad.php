...
class Salad extends Model {
	...
	public function saveImage($item, Request $request)
	{
		...
		{
			$file = $request->file('image')->move(static::imagePath(), 'salad-'.$item->id.".".Str::lower($request->file('image')->getClientOriginalExtension()));
	...
	{
		return config('laravel-glide.source.path').'/salads';
	}
