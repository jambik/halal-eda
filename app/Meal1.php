...
class Meal1 extends Model {
	...
	public function saveImage($item, Request $request)
	{
		...
		{
			$file = $request->file('image')->move(static::imagePath(), 'meal1-'.$item->id.".".Str::lower($request->file('image')->getClientOriginalExtension()));
	...
	{
		return config('laravel-glide.source.path').'/meal1';
	}
